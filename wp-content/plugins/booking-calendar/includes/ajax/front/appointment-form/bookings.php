...
$guest_booking = 1; //get_option( 'booked_booking_type', 'registered' ) === 'guest';
		...
		foreach ( $bookings as $calendar_id => $appointments ): ?>
			...
			<div class="booked-appointments">
				...
				foreach ($appointments as $appointment_key => $appointment):
					include( plugin_dir_path( __FILE__ ). 'appointment.php' );
					$appointment_counter++;
				endforeach;
				?>
			</div><?php
			...
			if ( get_option('users_can_register') && !$is_user_logged_in && $customer_type != 'guest' ) {
				...
				include( plugin_dir_path( __FILE__ ). 'form-fields-guest.php' );
			}
